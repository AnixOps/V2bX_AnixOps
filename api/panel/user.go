...
func (c *Client) GetUserList() ([]UserInfo, error) {
	const path = "/api/v2/server/UniProxy/user"
	r, err := c.client.R().
...
func (c *Client) GetUserAlive() (map[int]int, error) {
	c.AliveMap = &AliveMap{}
	const path = "/api/v2/server/UniProxy/alivelist"
	r, err := c.client.R().
...
func (c *Client) ReportUserTraffic(userTraffic []UserTraffic) error {
	...
	const path = "/api/v2/server/UniProxy/push"
	r, err := c.client.R().
		SetBody(data).
		ForceContentType("application/json").
		Post(path)
...
func (c *Client) ReportNodeOnlineUsers(data *map[int][]string) error {
	const path = "/api/v2/server/UniProxy/alive"
	r, err := c.client.R().
		SetBody(data).
		ForceContentType("application/json").
		Post(path)
