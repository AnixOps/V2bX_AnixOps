...
| Platform | Official plugin Supervisor and `nftables-forward` 1.1.0 | Preview/Partial | Signed artifact verification, node-isolated lifecycle journals, runtime-state/cleanup capabilities, canonical config validation, durable nftables ownership snapshot, TCP/UDP namespace traffic, exact rollback, and `SIGKILL` plus Agent-restart recovery | Keep `PluginSupervisorEnabled` opt-in; production requires matching Control topology admission, live kernel-observed health, staged fallback, rollout records, and a sustained canary. |
