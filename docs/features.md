...
|---------|--------|------------------------|
| P0 WireGuard dual-node relay | Partial/P0 | Initial entry WireGuard runtime, traffic delta parsing, and online-state reporting exist, but full GOST relay+QUIC routing, WSS compatibility, overseas exit NAT, speed limits, real integration tests, and GitHub Actions relay-path verification remain. |
